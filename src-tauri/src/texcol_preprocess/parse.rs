use crate::texcol_preprocess::ast::*;
use nom::branch::alt;
use nom::bytes::complete::{tag, take_while, take_while1};
use nom::character::complete::{char, digit1, none_of, one_of, satisfy, multispace1, multispace0};
...
use std::borrow::Cow;
use std::str::FromStr;
use nom::bytes::complete::take_until as nom_take_until;
...
// ignore conditions:
// TODO：支持IEEE，renewcommand等的ignore
...
// 用于跳过不关心的命令和内容
fn skip_irrelevant(input: &str) -> IResult<&str, ()> {
    let (input, _) = many0(alt((
        value((), tag("\\centering")), // 匹配 \centering，但不返回任何结果
        value((), multispace1),        // 匹配空白，但不返回任何结果
        // 你可以在这里添加更多的 pattern 来匹配和忽略其他内容
    )))(input)?;
...
/// for latex figure with the following format
/// 
/// \begin{figure}
///     \centering
///     \vspace{1cm}
...
///     \label{fig:label}
/// \end{figure}
pub fn figure_env(i: &str) -> Result<DocumentPart> {
    let (i, content) = raw_env("figure")(i)?;

    // centering
    let (content, _) = any_ws(content)?;
    let (content, _) = opt(command_no_args("centering"))(content)?;
    let (content, _) = any_ws(content)?; // \n
    // vspace

    let (content, _) = any_ws(content)?;
    // let (content, _) = opt(command("vspace", label_value))(content)?;
    let (content, _) = opt(raw_command("vspace"))(content)?;
    let (content, _) = any_ws(content)?; // \n

    // includegraphics
    let (content, _) = any_ws(content)?;

    let (content, _) = tag("\\includegraphics")(content)?;
    let (content, _) = delimited(char('['), nom_take_until("]"), char(']'))(content)?;

    let (content, img_path) = delimited(char('{'), nom_take_until("}"), char('}'))(content)?;
    // caption
    let (content, _) = any_ws(content)?;
    let (content, caption) = opt(command("caption", paragraph))(content)?;
    let (content, _) = any_ws(content)?;
    // label
    let (content, _) = any_ws(content)?;
    let (content, label) = opt(command("label", label_value))(content)?;

    Ok((i, DocumentPart::FigureDebug { img_path: img_path, caption: caption, label: label }))
}


pub fn table_env(i: &str) -> Result<DocumentPart> {
    let (i, _) = raw_env("table")(i)?;
    Ok((i, DocumentPart::Table))
}

pub fn table_star_env(i: &str) -> Result<DocumentPart> {
    let (i, _) = raw_env("table*")(i)?;
    Ok((i, DocumentPart::Table))
}
...
pub fn url_command(i: &str) -> Result<Url> {
    // let (i, val) = command("url", label_value)(i)?;
    let (i, url) = delimited(tag("\\url{"), nom_take_until("}"), char('}'))(i)?;
    Ok((i, Url(url)))
}
...
pub fn footnote(i: &str) -> Result<ParagraphPart> {
    let (i, content) = command("footnote",
...
pub fn paragraph<'a>(i: &'a str) -> Result<Paragraph<'a>> {
    ...
    let url_cmd = |i: &'a str| {
        let (i, url) = url_command(i)?;
        Ok((i, ParagraphPart::Url(url.0)))
    };
...
pub fn document_part<'a, 'b>(
    ...
    // add further parser for document part
...
pub fn preamble_lines<'a>(preamble: &'a str) -> Vec<&'a str> {
    let mut lines: Vec<&'a str> = Vec::new();
    let mut prev_line_was_ignore = false;
    for line in preamble.lines() {
        // TODO: Use a proper parser here.
        if line == "% LATEX_TO_HTML_IGNORE" {
            prev_line_was_ignore = true;
            continue;
        }

        if !prev_line_was_ignore {
            lines.push(line);
        }
        prev_line_was_ignore = false;
    }
...
pub fn document<'a>(i: &'a str) -> Result<Document<'a>> {
    ...
    let preamble = preamble_lines(preamble);
