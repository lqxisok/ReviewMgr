...
pub struct Analysis<'a> {
    ...
    pub math_image_text_source: HashMap<*const Math<'a>, String>,
...
impl<'a> Analysis<'a> {
    pub fn new(
        ...
        let math_image_text_source = math_image_text_source(doc, node_lists);
...
fn math_image_text_source<'a>(
    ...
    node_lists
        ...
        .map(|math| {
            match math {
                Math::Inline(src) => (addr_of!(*math), src.to_string()),
                Math::Display { source, .. } | Math::Mathpar { source, .. } => (addr_of!(*math), source.to_string()),
            }
        })
        .collect()
}
