...
use crate::texcol_preprocess::util::*;
use crate::texcol_preprocess::analysis::*;
...
fn display_math<'a>(analysis: &'a Analysis<'a>, math: &'a Math<'a>) -> impl 'a + Display {
    let src = analysis.math_image_text_source.get(&addr_of!(*math)).unwrap();
    let number: Option<&String> = analysis.math_numbering.get(&addr_of!(*math));
...
fn display_raw_math<'a>(analysis: &'a Analysis<'a>, math: &'a Math<'a>)  -> impl 'a + Display {
    let src: &String = analysis.math_image_text_source.get(&addr_of!(*math)).unwrap();
    DisplayFn(move|out: &mut Formatter| {
        use Math::*;
        match math {
            Inline(source) => {
            ...
            Display { source: str, label } | Mathpar { source: str, label } => {
    ...
    DisplayFn(move |out: &mut Formatter| {
        ...
        match part {
            ...
            Math(math) => {
                ...
                write!(out, "{}", display_raw_math(analysis, math))?;
            }
            Ref(value) => {
...
fn write_index(out: &mut impl Write, doc: &Document, analysis: &Analysis) -> Result {
    ...
    for part in doc.parts.iter() {
        ...
        match part {
            ...
            FigureDebug { img_path, caption, label } => {
                // TODO
            }
            Table => {
                // TODO
...
pub fn emit_to_string(doc: &Document, analysis: &Analysis) -> String{
    let mut index_src = String::new();
    write_index(&mut index_src, &doc, &analysis).unwrap();
    index_src
}